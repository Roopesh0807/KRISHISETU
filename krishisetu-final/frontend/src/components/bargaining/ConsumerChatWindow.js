...
const BargainChatWindow = () => {
  ...
  const [selectedProduct, setSelectedProduct] = useState(initialProduct || null);
  const [selectedQuantity, setSelectedQuantity] = useState(initialProduct?.minimum_quantity || '10');
  const [showPriceSuggestions, setShowPriceSuggestions] = useState(true);
  const [priceSuggestions, setPriceSuggestions] = useState([]);
  const [isTyping, setIsTyping] = useState(false);
  const [hasFarmerCounterOffer, setHasFarmerCounterOffer] = useState(false);
  const [isBargainComplete, setIsBargainComplete] = useState(false);
  const [bargainResult, setBargainResult] = useState(null);
  const [freezeUI, setFreezeUI] = useState(false);
  const [minPrice, setMinPrice] = useState(initialProduct?.minimum_price || 0);

  // Updated logic for generating price suggestions
  const generatePriceSuggestions = useCallback((currentBidPrice, minPrice, basePrice) => {
    const numericBid = parseFloat(currentBidPrice);
    const numericMin = parseFloat(minPrice);
    const numericBase = parseFloat(basePrice);
    
    // Validate inputs
    if (isNaN(numericMin) || isNaN(numericBase) || numericMin >= numericBase) {
        return [];
    }

    const suggestions = [];
    const lowerBound = numericMin;
    const upperBound = numericBid > numericMin ? numericBid : numericBase;
    const diff = upperBound - lowerBound;

    if (diff <= 0) return [];
    
    // Dynamically adjust number of suggestions
    const numSuggestions = Math.min(10, Math.floor(diff)) || 2;
    const stepSize = diff / (numSuggestions + 1);
    
    for (let i = numSuggestions; i >= 1; i--) {
        const newPrice = lowerBound + (stepSize * i);
        if (newPrice < upperBound) {
            suggestions.push({
                price: newPrice.toFixed(2),
                label: `₹${newPrice.toFixed(2)}`
            });
        }
    }
    
    const minPriceFixed = numericMin.toFixed(2);
    if (!suggestions.find(s => s.price === minPriceFixed)) {
        suggestions.push({
            price: minPriceFixed,
            label: `₹${minPriceFixed} (Min Price)`
        });
    }

    return suggestions.sort((a, b) => b.price - a.price);
  }, []);
  ...
  const fetchMessages = useCallback(async () => {
    try {
      ...
      console.error('Error fetching messages:', err);
      // It's important to set an error state here as well
  ...
  // Use a single useEffect for initial data fetching and socket setup
  useEffect(() => {
    let isMounted = true; 
    
    if (!bargainId || !token) {
        setLoading(false);
        setError("Missing bargain ID or token.");
        return;
    }
    ...
    const fetchDataAndConnect = async () => {
      ...
      try {
        const [sessionResponse, messagesResponse] = await Promise.all([
          fetch(`${process.env.REACT_APP_BACKEND_URL}/api/bargain/${bargainId}`, {
            headers: { 'Authorization': `Bearer ${token}` }
          }),
          fetch(`${process.env.REACT_APP_BACKEND_URL}/api/bargain/${bargainId}/messages`, {
            headers: { 'Authorization': `Bearer ${token}` }
          })
        ]);
        ...
        if (sessionData.success && sessionData.session && Array.isArray(sessionData.session.products) && sessionData.session.products.length > 0) {
          const product = sessionData.session.products[0];
          
          if (product && product.original_price && product.minimum_price && product.produce_name) {
            const mappedProduct = {
              ...product,
              price_per_kg: parseFloat(product.original_price),
              produce_name: product.produce_name,
            };

            setSelectedProduct(mappedProduct);
            const fetchedMinPrice = parseFloat(mappedProduct.minimum_price);
            const fetchedBasePrice = parseFloat(mappedProduct.price_per_kg);
            const fetchedCurrentPrice = parseFloat(mappedProduct.current_offer) || fetchedBasePrice;

            setCurrentPrice(fetchedCurrentPrice);
            setSelectedQuantity(mappedProduct.quantity || 1);
            setMinPrice(fetchedMinPrice);
            setBargainStatus(sessionData.session.status);

            if (sessionData.session.status === 'pending') {
              setPriceSuggestions(generatePriceSuggestions(fetchedBasePrice, fetchedMinPrice, fetchedBasePrice));
              setHasFarmerCounterOffer(false);
              setShowPriceSuggestions(true);
            } else {
              setFreezeUI(true);
              setIsBargainComplete(true);
              setBargainResult(sessionData.session.status);
              setShowPriceSuggestions(false);
              setHasFarmerCounterOffer(false);
            }

            if(sessionData.session?.farmer) {
              setSelectedFarmer(sessionData.session.farmer);
            }
            ...
            // Connect to WebSocket after all data is successfully loaded
            if (socket.current) {
                socket.current.disconnect();
            }

            const socketOptions = {
                auth: { token, bargainId },
                query: { bargainId, userType: 'consumer' },
                transports: ['websocket', 'polling'],
                reconnection: true,
                reconnectionAttempts: 5,
                reconnectionDelay: 1000,
            ...
            socket.current = io(process.env.REACT_APP_BACKEND_URL, socketOptions);
            
            // Set up socket listeners
            socket.current.on('connect', () => { setConnectionStatus("connected"); });
            socket.current.on('connect_error', (err) => { setConnectionStatus("error"); });
            socket.current.on('disconnect', (reason) => { setConnectionStatus("disconnected"); });
            socket.current.on('bargainMessage', (message) => {
                if (message?.message_content) {
                    setMessages(prev => [...prev, { ...message, content: message.message_content, timestamp: message.created_at, sender_type: message.sender_role }]);
                    if (message.sender_role === 'farmer' && message.price_suggestion) {
                        setCurrentPrice(message.price_suggestion);
                        setHasFarmerCounterOffer(true);
                        setWaitingForResponse(false);
                        setShowPriceSuggestions(false);
                        const suggestions = generatePriceSuggestions(message.price_suggestion, fetchedMinPrice, fetchedBasePrice);
                        setPriceSuggestions(suggestions);
                    }
                }
            });
...
socket.current.on('bargainStatusUpdate', (data) => {
    ...
    if (status === 'accepted' || status === 'rejected') {
        ...
        setPriceSuggestions(suggestions);
           ...
           setError('Failed to retrieve product details. The API response is malformed.');
        }
      } }catch (err) {
  ...
  }, [bargainId, token, generatePriceSuggestions, addSystemMessage, selectedFarmer]);
                        ...
                        priceSuggestions.map((suggestion, index) => (
                          ...
                          >
                            ...
                            <div className="krishi-suggestion-details">
                              <span className="krishi-suggestion-price">₹{suggestion.price}/kg</span>
                              <span className="krishi-suggestion-diff">
                                (₹{Math.abs(parseFloat(suggestion.price) - currentPrice).toFixed(2)} less)
                              </span>
                            </div>
                          </button>
                        ))
