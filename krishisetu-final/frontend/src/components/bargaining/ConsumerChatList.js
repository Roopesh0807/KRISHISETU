...
// // //       // Handle message update logic here
...
//       }
//     }
//     if (!normalized.farmer_name && (session.first_name || session.last_name)) {
//       normalized.farmer_name = `${session.first_name || ''} ${session.last_name || ''}`.trim() || defaultSession.farmer_name;
//     }
//     return normalized;
//   };
...
//         // Sort by current price descending for same product
...
//               bargain_id: session.bargain_id?.toString() || '',
//               farmer_id: session.farmer_id || '',
//               farmer_name: session.farmer_name || `${session.first_name || ''} ${session.last_name || ''}`.trim() || `Farmer ${session.farmer_id || 'Unknown'}`,
//               product_name: session.product_name || 'Unknown Product',
//               quantity: Number(session.quantity) || 0,
//               current_price: Number(session.current_price) || 0,
//               initial_price: Number(session.initial_price) || 0,
...
//               updated_at: session.updated_at || new Date().toISOString()
//             }))
//             .filter(session => session.bargain_id && session.farmer_id)
//         : [];
...
//   }, [consumerId, navigate, fetchSessions]);
...
//     navigate(`/bargain/${session.bargain_id}`, { state: { initialSession: session } });
//   };
...
//     session.farmer_name?.toLowerCase().includes(searchTerm.toLowerCase())
//   ) : bargainSessions;

//   const groupedSessions = groupSessionsByFarmer(filteredSessions);

//   const getBargainSuggestion = (session) => {
//     const isGoodDeal = session.current_price <= session.initial_price * 0.9;
//     if (session.status === 'pending' || session.status === 'countered') {
//       if (isGoodDeal) {
//         return <span style={{ color: 'green', fontWeight: 'bold' }}>Looks like a great deal!</span>;
//       } else {
//         return <span style={{ color: 'orange' }}>Check if you can get a better price.</span>;
//       }
//     }
//     return null;
//   };
...
//           <Link to="/consumer-dashboard" className="cart-icon-link"><FontAwesomeIcon icon={faShoppingCart} /></Link>
//         </div>
//         <div className="search-bar">
//           <input type="text" placeholder="Search products or farmers..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
//         </div>
...
//               <h3>No Bargain Requests</h3><p>Start a new bargain from a product listing.</p>
//             </div>
//           ) : (
...
//               return (
//                 <div key={group.farmer_id} className={`session-item ${selectedSession?.bargain_id === latestSession.bargain_id ? 'active' : ''}`} onClick={() => handleSessionClick(latestSession)}>
//                   <div className="session-info">
//                     <img src={group.farmer_profile_photo || 'https://via.placeholder.com/150'} alt={group.farmer_name} className="profile-photo" />
...
//             connectionStatus={connectionStatus}
//             initialSession={selectedSession}
//             onBack={() => { setSelectedSession(null); navigate("/consumer/bargain"); }}
//           />
//         ) : (
//           <div className="empty-chat-window">
//             <div className="empty-content">
...
const ConsumerChatList = () => {
  ...
  const fetchSessions = useCallback(async () => {
    try {
      ...
      const apiUrl = `${process.env.REACT_APP_API_BASE_URL}/api/bargain/consumers/${consumerId}/sessions`;
      console.log("API Endpoint:", apiUrl);
                        ...
                        {productCount === 1 && (
                          <>
                            ...
                            <span className="prices">
                              Base: ₹{latestSession.initial_price} | Current: ₹{latestSession.current_price}
                            </span>
                            <br/>
